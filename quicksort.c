...
/*                                                   +#+                      */
/*   Created: 2023/04/04 11:57:27 by henolimp      #+#    #+#                 */
/*   Updated: 2023/04/04 15:20:09 by henolimp      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_mediane(int *pivot, int *stack, int size)
{
	int	*tmp_stack;
	int	i;
...
int	ft_quicksort_a(t_stack *stack, int len, int cnt)
{
	...
	if (check_if_is_sorted(stack->a, len, 0) == 1)
		return(1);
	item = len;
	if(len <= 3)
	{
	...
	if (!ft_mediane (&pivot, stack->a, len))
		return (0);
	while (len != item / 2 + item % 2)
	{
	...
	while (item / 2 + item % 2 != stack->size_a && cnt--)
		ft_rra(stack, 1);
	return (ft_quicksort_a(stack, item / 2 + item % 2, 0)
			&& ft_quicksort_b(stack, item / 2, 0));
}
...
int	ft_quicksort_b(t_stack *stack, int len, int cnt)
{
	...
	if (!cnt && check_if_is_sorted(stack->b, len, 1) == 1)
		while (len--)
			ft_pa(stack, 1);
	if(len <= 3)
	{
	...
	item = len;
	if (!ft_mediane(&pivot, stack->b, len))
		return (0);
	while (len != item / 2)
	{
